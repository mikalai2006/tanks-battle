...
namespace Mikalai2006.Voxel
{
    ...
    public class Container : MonoBehaviour
    {
        ...
        public void Initialize(Material mat, Vector3 position, bool withCollider)
        {
            material = mat;
        ...
        public void SetData(SOVoxelData sOVoxelData, int indexGroup, float scale = 1)
        {
            ...
            // for (int j = 0; j < sOVoxelData.groups.Count; j++) {
                Vector3[] voxelList = sOVoxelData.groups[indexGroup].voxels.AsParallel().ToArray();
                ...
                for (int i = 0; i < voxelList.Length; i++)
                {
                    this[voxelList[i] * scale] = new Voxel()
                    {
                        ID = 1,
        ...
        public void GenerateMesh()
        {
            ...
            Debug.Log($"Create {meshData.vertices.Count} vertices, {meshData.triangles.Count} triangles");
        ...
        public Dictionary<Vector3, Voxel> ExposionVoxels(Vector3 _pointCollision, bool isDrawMesh, Collision _collision)
        {
            float startTime = Time.realtimeSinceStartup;
            ...
            Vector3[] keys = data.Keys.ToArray();
            NativeArray<Vector3> _needCreateElements = new NativeArray<Vector3>(keys.Length, Allocator.Persistent);
            NativeArray<Vector3> points = new NativeArray<Vector3>(keys.Length, Allocator.Persistent);
            ...
            JobHandle collisionJobHandle = collisionJob.Schedule(points.Length, 5);
            collisionJobHandle.Complete(); // Or use dependency


            for (int el = 0; el < collisionJob._needCreateElements.Length; el++)
            {
                if (collisionJob._needCreateElements[el] != Vector3.zero)
                {
                    needCreateElements.Push(collisionJob._needCreateElements[el]);
                    data.Remove(collisionJob._needCreateElements[el]);
                }
            }
        ...
        private IEnumerator createGO()
        {
            ...
            while (needCreateElements.Count > 0 && count > 0)
            {
                ...
                Vector3 pointSpawnVoxel = collision.gameObject.transform.TransformPoint(elem);
    ...
    [BurstCompile]
        struct CheckCollisionJob : IJobParallelFor
        {
           ...
           public NativeArray<Vector3> _needCreateElements;

            public void Execute(int index)
            {
                Vector3 point = points[index];
                if (TestHelpers.IsInsideSphere(point, _pointCollision, 6))
                {
                    // list.Add(posx, data.ElementAt(j).Value);
                    // // data[posx] = new Voxel()
                    // // {
                ...
                else
                {
                    ...
                    _needCreateElements[index] = Vector3.zero;
                }
            }
        }
